...
@interface OPTLYFMDBDatabaseQueueTests : OPTLYFMDBTempDBTests

@property OPTLYFMDBDatabaseQueue *queue;

@end

@implementation OPTLYFMDBDatabaseQueueTests
...
- (void)setUp
{
    ...
    self.queue = [OPTLYFMDBDatabaseQueue databaseQueueWithPath:self.databasePath];
}
...
- (void)testURLOpenNoPath {
    OPTLYFMDBDatabaseQueue *queue = [[OPTLYFMDBDatabaseQueue alloc] init];
    XCTAssert(queue, @"Database queue should be returned");
    queue = nil;
}

- (void)testURLOpenNoURL {
    OPTLYFMDBDatabaseQueue *queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:nil];
    XCTAssert(queue, @"Database queue should be returned");
    queue = nil;
}
...
- (void)testURLOpen {
    ...
    OPTLYFMDBDatabaseQueue *queue = [OPTLYFMDBDatabaseQueue databaseQueueWithURL:fileURL];
    XCTAssert(queue, @"Database queue should be returned");
    queue = nil;
...
- (void)testURLOpenInit {
    ...
    OPTLYFMDBDatabaseQueue *queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:fileURL];
    XCTAssert(queue, @"Database queue should be returned");
    queue = nil;
    [[NSFileManager defaultManager] removeItemAtURL:fileURL error:nil];
}
...
- (void)testURLOpenWithOptions {
    ...
    OPTLYFMDBDatabaseQueue *queue = [OPTLYFMDBDatabaseQueue databaseQueueWithURL:fileURL flags:SQLITE_OPEN_READWRITE];
...
- (void)testURLOpenInitWithOptions {
    ...
    OPTLYFMDBDatabaseQueue *queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:fileURL flags:SQLITE_OPEN_READWRITE];
    XCTAssertNil(queue, @"Database queue should not have been created");

    queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:fileURL flags:SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE];
    XCTAssert(queue, @"Database queue should have been created");
    
    [queue inDatabase:^(OPTLYFMDBDatabase * _Nonnull db) {
    ...
    queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:fileURL flags:SQLITE_OPEN_READONLY];
...
- (void)testURLOpenWithOptionsVfs {
    ...
    OPTLYFMDBDatabaseQueue *queue = [[OPTLYFMDBDatabaseQueue alloc] initWithURL:fileURL flags:SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE vfs:@"MyCustomVFS"];
...
{
    [self.queue inDatabase:^(OPTLYFMDBDatabase *adb) {
        int count = 0;
        OPTLYFMDBResultSet *rsl = [adb executeQuery:@"select * from qfoo where foo like 'h%'"];
        while ([rsl next]) {
            count++;
        }
...
{
    OPTLYFMDBDatabaseQueue *queue2 = [OPTLYFMDBDatabaseQueue databaseQueueWithPath:self.databasePath flags:SQLITE_OPEN_READONLY];
    XCTAssertNotNil(queue2);

    {
        [queue2 inDatabase:^(OPTLYFMDBDatabase *db2) {
            OPTLYFMDBResultSet *rs1 = [db2 executeQuery:@"SELECT * FROM qfoo"];
            XCTAssertNotNil(rs1);
...
- (void)testStressTest
{
    ...
    dispatch_apply(ops, dqueue, ^(size_t nby) {
        ...
        if (nby % 2 == 1) {
            ...
            [self.queue inTransaction:^(OPTLYFMDBDatabase *adb, BOOL *rollback) {
                OPTLYFMDBResultSet *rsl = [adb executeQuery:@"select * from qfoo where foo like 'h%'"];
                while ([rsl next]) {
                    ;// whatever.
                }
...
- (void)testTransaction
{
    [self.queue inDatabase:^(OPTLYFMDBDatabase *adb) {
        ...
        int rowCount = 0;
        OPTLYFMDBResultSet *ars = [adb executeQuery:@"select * from transtest"];
        while ([ars next]) {
            rowCount++;
        }
